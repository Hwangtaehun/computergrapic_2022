	...
	const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	polygonLayout[1].SemanticName = "TEXCOORD";  //color에서 texture로 변경
	...
	samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;  //필터 설정(triliner)
											 ...
											 XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	deviceContext->PSSetShaderResources(0, 1, &texture); //추가 정보 shader적용할 텍스쳐 정보 -> &texture 중요 0,1은 건들이지 않음
...
void TextureShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	deviceContext->PSSetSamplers(0, 1, &m_sampleState);  //필터 -> Point, Bilinear, Trilinear, Anisotropic 중 하나 선택
