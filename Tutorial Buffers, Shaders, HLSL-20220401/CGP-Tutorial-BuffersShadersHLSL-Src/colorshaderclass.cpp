...
ColorShaderClass::ColorShaderClass()
{
	...
	m_pixelShader = 0;
	m_layout = 0;
	m_matrixBuffer = 0; //vertex, matrix 삼총사
}
...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./data/color.vs", L"./data/color.ps"); //위치를 알려준다.
	if(!result)
	{
		return false;
	}
...
// 원하는 자료나 특성을 가진 커스텀 정점형식(구조체를 만듦)을 만들려면 우선 그러한 자료를 담을 구조체를 정의해야함.
...
// (예를 들어 정점 구조체의 성분이 두 개라고 하면, D3D11_INPUT_ELEMENT_DESC 배열의 원소도 두개이어야 함.)
	...
	const WCHAR *vsFilename, const WCHAR *psFilename)
{
	...
	D3D11_INPUT_ELEMENT_DESC polygonLayout[2]; //지금은 position, color 뿐이지만 나중에 더 늘어남
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "ColorVertexShader", "vs_5_0", //.vs, .ps를 만들어내는 부분/DX11이므로 5.0버전으로 사용
		D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);

	if(FAILED(result)) //실패시 에러메시지
	{
    ...
    // Create the vertex shader from the buffer. 수정은 없다.
    ...
    // Create the pixel shader from the buffer. 수정할 필요 없다.
	...
	// 성분의 시작 위치 사이의 오프셋(바이트 단위)임. + 채널을 만들고 데이터 넘겨주기 / GPU가 받을 수 있도록 패킹하는 부분 polygonLayout
	polygonLayout[0].SemanticName = "POSITION";															/*시작 이클래스에 건드릴 부분*/
	polygonLayout[0].SemanticIndex = 0;
	polygonLayout[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;  //positon의 크기 RBG float 사용 32 32 32	/*끝*/
	...
	polygonLayout[1].Format = DXGI_FORMAT_R32G32B32A32_FLOAT; // color의 크기 32 32 32 32크기로 구성
	...
	// Create the vertex input layout. -> GPU쪽에 레이아웃 만들기 
    ...
    // Setup the description of the dynamic matrix constant buffer that is in the vertex shader. ->기하학 배열 삼총사 정보를 GPU 넣기
	matrixBufferDesc.Usage = D3D11_USAGE_DYNAMIC;				// The buffer is updated each frame. ->계속 업데이트 정보이므로 DYNAMIC(동적)
	matrixBufferDesc.ByteWidth = sizeof(MatrixBufferType);		// 크기 중요
...
// vertex shader during the Render function call. -> 값전달 함수 D3D클라스와 카메라클라스 가진 값을 전달
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	// Transpose the matrices to prepare them for the shader. 전치-> 행렬의 열과 행이 뒤바뀌어 있어서 전치한다.-> 치환해야한다.
	worldMatrix = XMMatrixTranspose(worldMatrix);
	viewMatrix = XMMatrixTranspose(viewMatrix);
	projectionMatrix = XMMatrixTranspose(projectionMatrix);

	// Lock the constant buffer so it can be written to.
	result = deviceContext->Map(m_matrixBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource); //map이라는 함수를 통해서 열어서 Geometry정보를 쉐이더 함수로 전달
	if(FAILED(result))
	{
		return false;
	}
	...
	// Unlock the constant buffer. map 무조건 닫아주어야한다. 그 함수가 upmap
...
// DrawIndexed DirectX 11 function using the D3D device context. 맨 마지막에 실행
