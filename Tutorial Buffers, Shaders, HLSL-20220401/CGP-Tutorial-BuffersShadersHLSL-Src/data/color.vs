...
/////////////
cbuffer MatrixBuffer  //shader에서는 class를 사용하지 않는다. 자주 바뀌기 때문에 전역으로 사용함
{
	matrix worldMatrix;
	matrix viewMatrix;
...
{
    float4 position : POSITION;  /*semantic-> POSITION, COLOR 타입을 나타냄, 동차좌표 vs에서 사용*/
    float4 color : COLOR;
};

struct PixelInputType
{
    float4 position : SV_POSITION; /*plxel shider 사용하기위해서 SV_POSITION*/
    float4 color : COLOR;
};
...
////////////////////////////////////////////////////////////////////////////////
PixelInputType ColorVertexShader(VertexInputType input) //ColorVertexShader 내용 -> GPU가 처리 할 수 있게 Shader에 넣는 과정 -> Vertex를 변환Vertex로 바뀌는 일을 한다.
{
    PixelInputType output;
    

	// Change the position vector to be 4 units for proper matrix calculations.
    input.position.w = 1.0f;  //포지션으로 사용하겠다.

	// Calculate the position of the vertex against the world, view, and projection matrices.
    output.position = mul(input.position, worldMatrix);  //mul: 백터와 행렬을 곱하는 함수
    output.position = mul(output.position, viewMatrix);
    output.position = mul(output.position, projectionMatrix);
    
	// Store the input color for the pixel shader to use.
    output.color = input.color;  /*Geometry 영역에는 컬러 건드릴 필요없음*/
