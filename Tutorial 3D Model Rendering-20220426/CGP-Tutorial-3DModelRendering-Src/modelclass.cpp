...
bool ModelClass::Initialize(ID3D11Device* device, const WCHAR* modelFilename, const WCHAR* textureFilename)
{
	...
	result = LoadModel(modelFilename);  //새로운 함수 추가
	if (!result)
	{
		return false;
	}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	// Load the vertex array and index array with data.->로딩타임 오래 걸리때 수정하기
...
bool ModelClass::ReadFileCounts(const WCHAR* filename)
{
	ifstream fin;
	char input;
	// Initialize the counts.->해당 파일을 열어봤을 때 몇개인지 숫자만 확인한다.
	int vertexCount = 0;
	int textureCount = 0;
	int normalCount = 0;
	int faceCount = 0;
	...
	while (!fin.eof())
	{
		// If the line starts with 'v' then count either the vertex, the texture coordinates, or the normal vector. -> parsing 할때 확인
		if (input == 'v')
		{
			fin.get(input);
		...
		if (input == 'v')
		{
			...
			if (input == ' ')
			{
				fin >> vertices[vertexIndex].x >> vertices[vertexIndex].y >>
					vertices[vertexIndex].z;  //obj 안되면 수정
			...
			if (input == 't')
			{
				...
				// Invert the V texture coordinates to left hand system. 중요
				texcoords[texcoordIndex].y = 1.0f - texcoords[texcoordIndex].y; //음수가 나오면 안된다.
				texcoordIndex++;
			}
