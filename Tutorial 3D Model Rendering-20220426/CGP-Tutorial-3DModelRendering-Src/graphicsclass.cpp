...
bool GraphicsClass::Frame()
{
	...
	static float rotation = 0.0f;  //회전


	// Update the rotation variable each frame. -> 회전 관련
	rotation += (float)XM_PI * 0.01f; //-값을 주면 반대방향으로 간다.
	if (rotation > 360.0f)
	{
		rotation -= 360.0f;
	}
...
bool GraphicsClass::Render(float rotation)
{
	...
	worldMatrix = XMMatrixRotationY(rotation); //각자 돌리하는 법 생각해보기
