...
bool ModelClass::InitializeBuffers(ID3D11Device* device) //인스턴스 버퍼가 이안에 있음
{
	// 정점 배열의 정점 수를 설정합니다.
	m_vertexCount = 3;
    ...
    instanceBufferDesc.Usage = D3D11_USAGE_DEFAULT; //중요
    instanceBufferDesc.ByteWidth = sizeof(InstanceType) * m_instanceCount;  //중요
...
{
	// 버퍼 오프셋과 스트라이드를 설정합니다. 배열로 담음, 추가적인 상황 -> 한개로 묶어서 보냄
