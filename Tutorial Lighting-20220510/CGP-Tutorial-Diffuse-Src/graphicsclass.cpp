...
//	m_Camera->SetPosition(0.0f, 0.5f, -3.0f);	// for chair
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), L"./data/cube.obj", L"./data/seafloor.dds");  //큐브 띠움
//	result = m_Model->Initialize(m_D3D->GetDevice(), L"./data/chair.obj", L"./data/chair_d.dds");
	...
	m_LightShader = new LightShaderClass;	//라이트 쉐이더
	if (!m_LightShader)
	{
		return false;
	}
	...
	m_Light->SetDiffuseColor(1.0f, 1.0f, 1.0f, 1.0f); //하얀색으로 함
	m_Light->SetDirection(0.0f, 0.0f, 1.0f);		  //왼쪽 손 법칙으로 정면으로 비춤 /오른쪽 방향(1.0f , 0.0f, 0.0f)
